class_name GSConstants
## Various constants used by [GSClient].
##
## Defines various default values and project settings used by [GSClient] and gspot in general.

const CLIENT_NAME: String = "GSClient"
const CLIENT_VERSION: String = "3.0.0"
const MESSAGE_VERSION: int = 3
const MESSAGE_RATE: float = 0.2
const PROJECT_SETTING_CLIENT_NAME: String = "gspot/client/client_name"
const PROJECT_SETTING_CLIENT_VERSION: String = "gspot/client/client_version"
const PROJECT_SETTING_ENABLE_RAW_COMMANDS: String = "gspot/client/enable_raw_commands"
const PROJECT_SETTINGS_MESSAGE_RATE: String = "gspot/client/message_rate"
const PROJECT_SETTING_PROTOCOL_MODE: String = "gspot/client/protocol_mode"
